...
function MUISwitch(props: UseSwitchProps) {
  ...
  const [t, setTheme] = usePersistedState("theme", "light");
  ...
  return (
    <Box sx={{ display: {  md: "flex",width:"65px", marginRight:"0px" } }}>
...
export default function UseSwitchesCustom() {
  const [theme_value, t] = usePersistedState("theme", "light");

  return <MUISwitch defaultChecked={theme_value === "dark"} />;
}
...
position: absolute;
display: block;
background-color: ${theme.palette.mode === "dark" ? "#121212" : "#344CB7"};
width: 28px;
height: 28px;
border-radius: 14px;
top:  1px;
